    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
      ...
      <section class="mb-12">
        <div class="text-center mb-8">
          <h2 class="text-3xl sm:text-4xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent tracking-tight">
            Discover Events
          </h2>
          <p class="mt-4 max-w-2xl mx-auto text-lg text-purple-200">
            Find your next immersive experience
          </p>
        </div>
        ...
        <div class="max-w-3xl mx-auto mb-6">
          <div class="relative">
            <svg 
              class="absolute left-5 top-1/2 transform -translate-y-1/2 w-5 h-5 text-purple-300"
              fill="none" 
              stroke="currentColor" 
              viewBox="0 0 24 24"
            >
            ...
            <input 
              ...
              class="w-full pl-14 pr-24 py-4 text-lg bg-slate-800/50 backdrop-blur-xl border-2 border-purple-500/30 rounded-full text-white placeholder-purple-300/50 focus:outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-500/50 transition-all"
            />
            <button 
              v-if="searchQuery"
              @click="searchQuery = ''"
              class="absolute right-16 top-1/2 transform -translate-y-1/2 text-purple-300 hover:text-white p-2 rounded-full hover:bg-purple-500/20 transition-all"
            ...
            <button class="absolute right-2 top-1/2 transform -translate-y-1/2 bg-gradient-to-r from-purple-600 to-pink-600 text-white p-3 rounded-full hover:from-purple-500 hover:to-pink-500 transition-all shadow-lg shadow-purple-500/50">
              🔍
            </button>
          </div>
        </div>
          ...
          <button 
            ...
            :class="[
              ...
              (cat === 'All' && !selectedCategory) || selectedCategory === cat
                ...
                : 'bg-slate-800/50 text-purple-200 border border-purple-500/30 hover:bg-slate-700/50 backdrop-blur-sm'
            ]"
          >
            {{ cat }}
          </button>
        ...
        <div class="flex flex-wrap justify-center gap-4 items-center">
          <div class="flex items-center gap-2">
            <label class="text-purple-200 font-semibold">Date:</label>
            <input 
              v-model="selectedDate" 
              type="date" 
              class="px-4 py-2 bg-slate-800/50 border-2 border-purple-500/30 rounded-lg text-white focus:outline-none focus:border-purple-500 backdrop-blur-sm"
            />
          </div>
          <button 
            v-if="searchQuery || selectedCategory || selectedDate"
            @click="clearFilters"
            class="px-4 py-2 bg-red-500/20 text-red-200 border border-red-500/30 rounded-lg font-semibold hover:bg-red-500/30 transition-all backdrop-blur-sm"
          >
            Clear Filters
          </button>
        </div>
      ...
      <hr class="border-purple-500/20 my-8" />
      ...
      <section class="my-16">
        <div class="text-center mb-8">
          ...
          <p class="mt-4 max-w-2xl mx-auto text-lg text-purple-200">
            ...
            <span v-else-if="searchQuery || selectedCategory || selectedDate">
              ...
              <span v-if="searchQuery" class="font-semibold text-purple-400">
                for "{{ searchQuery }}"
              </span>
            </span>
            <span v-else>
        ...
        <!-- Events Grid - Responsive: 1 col mobile, 2 col tablet, 3 col desktop, 4 col large -->

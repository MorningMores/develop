...
describe('ProfilePage.vue', () => {
  ...
  // ==================== Branch Coverage Tests for Profile Data Rendering ====================

  it('should render profile data when available', async () => {
    const profileData = {
      firstName: 'John',
      lastName: 'Doe',
      phone: '1234567890',
    ...
    // Check profile data is rendered
    const html = wrapper.html()
  ...
  it('should show "No personal info yet" when profile is null', async () => {
    ...
    expect(wrapper.html()).toContain('No personal info yet')
  })

  it('should handle profile with only some fields populated', async () => {
    const partialProfile = {
      firstName: 'Alice',
      phone: '9876543210'
      // Missing lastName, address, city, country, pincode
    }
    
    localStorage.setItem('profile_data', JSON.stringify(partialProfile))
    ;(global.$fetch as any).mockResolvedValueOnce({
      username: 'alice',
  ...
  it('should render Edit Profile link when profile data loads', async () => {
    ;(global.$fetch as any).mockResolvedValueOnce({
      username: 'testuser',
      email: 'test@example.com'
    })
    ...
    const link = wrapper.find('a[to="/AccountPage"]')
    expect(link.exists()).toBe(true)
    expect(link.text()).toContain('Edit Profile')
  })

  it('should handle profile data with empty strings', async () => {
    const emptyProfile = {
      firstName: '',
      lastName: '',
      phone: '',
    ...
    localStorage.setItem('profile_data', JSON.stringify(emptyProfile))
    ;(global.$fetch as any).mockResolvedValueOnce({
      username: 'emptyuser',
    ...
    // Should show "-" for all empty fields
    const html = wrapper.html()
    expect(html.match(/-/g)?.length).toBeGreaterThanOrEqual(5)
  })

  it('should handle JSON parse error for profile_data', async () => {
    ...
    // Should handle gracefully and show "No personal info yet"
    expect(wrapper.html()).toContain('No personal info yet')
  })

  it('should display username with fallback to "-"', async () => {
    ;(global.$fetch as any).mockResolvedValueOnce({
      username: '',
      email: 'test@example.com'
    })
  ...
  it('should display email with fallback to "-"', async () => {
    ;(global.$fetch as any).mockResolvedValueOnce({
      username: 'testuser',
      email: ''
    })

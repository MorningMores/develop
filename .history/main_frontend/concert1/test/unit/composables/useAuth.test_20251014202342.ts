...
describe('useAuth Composable', () => {
  ...
  describe('Token Management', () => {
    ...
    it('should store token in sessionStorage when remember is false', () => {
      ...
      expect(sessionStorage.setItem).toHaveBeenCalledWith('jwt_token', 'session-token')
    })
  ...
  describe('Logout', () => {
    ...
    it('should clear token from sessionStorage', () => {
      ...
      const { logout } = useAuth()
      logout()
      expect(sessionStorage.removeItem).toHaveBeenCalledWith('jwt_token')
    })
    ...
    it('should clear profile data on logout', () => {
      ...
      const { logout } = useAuth()
      logout()
      expect(localStorage.removeItem).toHaveBeenCalledWith('profile_data')
    })
  })
  ...
  describe('Profile Management', () => {
    it('should save profile data', () => {
      ...
      expect(localStorage.setItem).toHaveBeenCalledWith('profile_data', JSON.stringify(profileData))
    })

    it('should retrieve profile data', () => {
      const profileData = { username: 'testuser', email: 'test@test.com' }
      localStorage.getItem = vi.fn().mockReturnValue(JSON.stringify(profileData))

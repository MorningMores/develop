...
const selectedFilter = ref('All') // New filter state for the buttons
...
const filteredEvents = computed(() => {
  ...
  const tomorrow = new Date(today).setDate(now.getDate() + 1)
  
  if (selectedFilter.value === 'Today') {
    result = result.filter(e => new Date(e.startDate || e.datestart).setHours(0,0,0,0) === today)
  } else if (selectedFilter.value === 'Tomorrow') {
    result = result.filter(e => new Date(e.startDate || e.datestart).getTime() >= tomorrow && new Date(e.startDate || e.datestart).getTime() < tomorrow + 24 * 60 * 60 * 1000)
...
// Helper to format date for the cards
